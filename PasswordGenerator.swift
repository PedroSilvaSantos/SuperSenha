...
class PasswordGenerator {
    ...
    init(numberOfCharacaters: Int, useLetters: Bool, useNumbers: Bool, useCapitalLetters: Bool, useSpecialCharacters: Bool) {
        ...
        numchars = max(numchars, 6) //irei receber o meu proprio valor, ou 1 senha apenas
    ...
    func generate(total: Int) -> [String] {
        ...
        if universeArray.count > 0 {
            while passwords.count < total { //total = quantidade de senhas solicitadas
